...
#[derive(Clone)]
pub struct AncestorsIter<'a, S: ForestNodeStore + ?Sized> {
    store: &'a S,
    /// The next node ID to yield. `None` when the root has been yielded.
    current: Option<TreeNodeId>,
}

impl<'a, S: ForestNodeStore + ?Sized> AncestorsIter<'a, S> {
...
impl<'a, S: ForestNodeStore + ?Sized> Iterator for AncestorsIter<'a, S> {
...
#[derive(Clone)]
pub struct BfsIter<'a, S: ForestNodeStore + ?Sized>
where
...
impl<'a, S: ForestNodeStoreDown + ?Sized> BfsIter<'a, S> {
...
impl<'a, S: ForestNodeStoreDown + ?Sized> Iterator for BfsIter<'a, S> {
    type Item = TreeNodeId;
    fn next(&mut self) -> Option<Self::Item> {
...
#[derive(Clone)]
pub struct PreorderIter<'a, S: ForestNodeStore + ?Sized>
where
...
impl<'a, S: ForestNodeStoreDown + ?Sized> PreorderIter<'a, S> {
...
impl<'a, S: ForestNodeStoreDown + ?Sized> Iterator for PreorderIter<'a, S> {
    type Item = TreeNodeId;
    fn next(&mut self) -> Option<Self::Item> {
